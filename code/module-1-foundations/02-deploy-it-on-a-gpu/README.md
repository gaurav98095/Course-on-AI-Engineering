...
Self-contained: this folder is a **copy-forward** of `01-build-a-multimodal-rag/` (same code, same data) plus an HTTP endpoint. Clone the repo, `cd` here, and everything runs.
...
| File | What it is |
| --- | --- |
| `serve.py` | LitServe HTTP endpoint — the model loads once in `setup`, then answers forever |
| `client.py` | Calls the endpoint from anywhere, prints timing breakdown |

(`ingest.py`, `rag.py`, `measure.py`, and `data/` are unchanged from Lecture 01 — copied here so this folder runs on its own.)
...
⚡ Terminal 2 — health check and a real call:
...
Previous: [`../01-build-a-multimodal-rag/`](../01-build-a-multimodal-rag/) · Next: [`../03-load-test-it-until-it-breaks/`](../03-load-test-it-until-it-breaks/) — the same endpoint, under siege.
