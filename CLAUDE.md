...
- Math delimiters: always write inline math as `\(...\)` and display math as `\[...\]` — never `$...$` or `$$...$$`. Kramdown's `$`-based math parsing is unreliable (it inconsistently converts or silently leaves LaTeX as literal text), and this site's prose also quotes real dollar amounts ("~$1/hr"), so a `$`-delimiter is unsafe. `\(\)`/`\[\]` pass through kramdown untouched and are MathJax's default delimiters — no config needed.
